...
def test_runpod_vllm_native_poll_timeout_includes_cold_start(monkeypatch) -> None:
    monkeypatch.delenv("GPUCALL_RUNPOD_VLLM_NATIVE_POLL_TIMEOUT_SECONDS", raising=False)
    monkeypatch.delenv("GPUCALL_RUNPOD_VLLM_NATIVE_RUNTIME_MULTIPLIER", raising=False)
    monkeypatch.setenv("GPUCALL_RUNPOD_VLLM_NATIVE_POLL_MIN_SECONDS", "60")
    monkeypatch.setenv("GPUCALL_RUNPOD_VLLM_NATIVE_POLL_MAX_SECONDS", "600")
    ...
    assert _runpod_vllm_native_poll_timeout_seconds(plan) == 316
...
@pytest.mark.asyncio
async def test_runpod_vllm_health_and_models_404_falls_back_to_chat_completion(monkeypatch) -> None:
    calls: list[str] = []
        ...
        def post(self, url: str, **_kwargs):
            ...
            return FakeResponse(
                200,
                {
                    "choices": [
                        {
                            "message": {"content": "runpod chat ok"},
                            "finish_reason": "stop",
                        }
                    ]
                },
    ...
    assert result.value == "runpod chat ok"
    assert calls == [
        "GET https://api.runpod.ai/v2/endpoint-1/health",
        "GET https://api.runpod.ai/v2/endpoint-1/openai/v1/models",
        "POST https://api.runpod.ai/v2/endpoint-1/openai/v1/chat/completions",
