    ...
    def _provision_and_start(self, plan: CompiledPlan) -> dict[str, Any]:
        ...
        try:
            ...
            cmd = (
                ...
                "vllm_pkg=os.environ.get('GPUCALL_HYPERSTACK_VLLM_PACKAGE','vllm==0.7.3')\n"
                "transformers_pkg=os.environ.get('GPUCALL_HYPERSTACK_TRANSFORMERS_PACKAGE','transformers==4.51.3')\n"
...
def artifact_dek_bytes():
    ...
    if path:
        ...
        return data
    if os.environ.get("GPUCALL_ALLOW_ARTIFACT_DEK_ENV", "").strip().lower() not in {"1", "true", "yes", "on"}:
