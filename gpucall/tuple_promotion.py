...
def promote_production_tuple(
    *,
    ...
    validation_budget_usd: float | None = None,
    activate: bool = False,
    force: bool = False,
) -> dict[str, Any]:
    ...
    promotion_report: dict[str, Any] = {
        ...
        "next_actions": [
            ...
            f"run gpucall tuple-smoke {tuple['name']} --config-dir {promotion_config} --recipe {recipe['name']} --mode {validation_mode} --budget-usd <limit> --write-artifact",
    ...
    if run_validation:
        if validation_budget_usd is None:
            raise ValueError("--run-validation requires --validation-budget-usd")
        validation = _run_tuple_validation(
            tuple["name"],
            recipe["name"],
            promotion_config,
            mode=validation_mode,
            validation_dir=validation_dir,
            budget_usd=validation_budget_usd,
        )
...
def _tuple_from_candidate(candidate: Mapping[str, Any], *, active_config: Any) -> dict[str, Any]:
    ...
    source: dict[str, Any] = {
        ...
        "estimated_prefill_tokens_per_second": candidate.get("estimated_prefill_tokens_per_second"),
        "estimated_decode_tokens_per_second": candidate.get("estimated_decode_tokens_per_second"),
        "estimated_runtime_overhead_seconds": candidate.get("estimated_runtime_overhead_seconds"),
        "runtime_estimate_safety_multiplier": candidate.get("runtime_estimate_safety_multiplier"),
    ...
    surface = _drop_none(
        {
            ...
            "expected_runtime_seconds": tuple.get("expected_runtime_seconds"),
            "estimated_prefill_tokens_per_second": tuple.get("estimated_prefill_tokens_per_second"),
            "estimated_decode_tokens_per_second": tuple.get("estimated_decode_tokens_per_second"),
            "estimated_runtime_overhead_seconds": tuple.get("estimated_runtime_overhead_seconds"),
            "runtime_estimate_safety_multiplier": tuple.get("runtime_estimate_safety_multiplier"),
