...
def generate_tuple_candidates(snapshot: ResourceCatalogSnapshot, *, recipe: Recipe | None = None) -> list[TupleCandidate]:
    ...
    for resource in snapshot.resources:
        ...
        payload = {
            ...
            "modes": worker.modes,
            "production_state": "production_configured" if resource.source == "active_provider" else "candidate_draft",
...
def _active_provider_payload(provider: Any) -> dict[str, Any]:
    payload = provider.model_dump(mode="json")
    payload["source"] = "active_provider"
    return payload
...
def _resource_entry(row: Mapping[str, Any]) -> ResourceCatalogEntry:
    source = "provider_candidate" if row.get("source") == "provider_candidate" else "active_provider"
    ...
    return ResourceCatalogEntry(
        ...
        stock_state="candidate" if source == "provider_candidate" else "configured",
    )
...
def _config_hash(*, config: GpucallConfig, candidate_rows: list[Mapping[str, Any]]) -> str:
    payload = {
        ...
        "providers": {key: value.model_dump(mode="json") for key, value in sorted(config.providers.items())},
        ...
        "provider_candidates": sorted(candidate_rows, key=lambda row: str(row.get("name") or "")),
    }
